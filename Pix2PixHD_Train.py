...
# [1] Initial Conditions Setup
    ...
    dtype = torch.float16 if opt.data_type == 16 else torch.float64
    ...
    # for data in dataset:
    #   print(data[0].shape)
    #   # assert data[0].shape[1] == 25
    #   # break
    src_demo_data = torch.rand((1,25,1024,1024))
    tgt_demo_data = torch.rand((1, 4, 1024, 1024))
...
# [2] Model training
    ...
    for epoch in range(init_epoch, opt.n_epochs + 1):
        print('print cuda memory allocated ', torch.cuda.memory_allocated())
...
# [2] Model Checking 
            ...
            if opt.val_during_train and (current_step % save_freq == 0):
                ...
                for input, target, _, name in tqdm(test_data_loader):
                    ...
                    manager.save_image(np_fake, path=os.path.join(test_image_dir, 'Check_{:d}_'.format(current_step)+ name[0] + '_fake'))
                    manager.save_image(np_real, path=os.path.join(test_image_dir, 'Check_{:d}_'.format(current_step)+ name[0] + '_real'))
